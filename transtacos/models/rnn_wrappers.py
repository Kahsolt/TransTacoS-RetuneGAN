...
class FrameProjection:
  """Projection layer to r * n_mel dimensions or n_mel dimensions"""
  def __init__(self, shape=hp.n_mel, activation=None, scope='linear_projection'):
    """
    Args:
...
class StopProjection:
  ...
  def __init__(self, is_training, shape=1, activation=tf.nn.sigmoid, scope='stop_token_projection'):
    """
    Args:
...
class TacotronDecoderCellState(
  namedtuple("TacotronDecoderCellState",
...
class TacotronDecoderWrapper(RNNCell):
  ...
  * : This is typically taking a vanilla LSTM, wrapping it using tensorflow's attention wrapper,
  ...
  def __init__(self, rnn_cell, attention_mechanism, frame_projection, stop_projection, is_training):
    ...
    Args:
        ...
        frame_projection: tensorflow fully connected layer with r * n_mel output units
  ...
  def __call__(self, inputs, state):
    ...
    # just adjust n_mel to encoder_depth (?)
    prenet_output = prenet(inputs, hp.prenet_depths, self._training, scope='decoder_prenet')
    ...
    alignments, cumulated_alignments = self._attention_mechanism(rnn_output, state=previous_alignments)
    ...
    context = tf.matmul(expanded_alignments, self._attention_mechanism.values)
    context = tf.squeeze(context, [1])    # V

    #Concat RNN outputs and context vector to form projections inputs
    projections_input = tf.concat([rnn_output, context], axis=-1)
